...
# - Immutable: 새 값 할당 시 객체 식별자 변경, Mutable: 새 값을 할당 시 객체 식별자 유지
...
# soft copy : 리스트 변수를 다른 변수에 그대로 할당 -> 객체 식별자 값 공유, 한 변수의 값 변경 시 다른 변수에도 영향을 미침
# deep copy : pockets_copy = pockets[:] -> 서로 다른 객체 식별자 값, 메모리 상에서 독립적으로 존재
lst = [5]
soft_copy = lst
soft_copy.append(3)
print(lst)  # [ 5, 3 ]
print(id(lst) == id(soft_copy))

hard_copy = lst[:]
hard_copy.append(6)
print(lst)          # lst = [ 5, 3 ]
print(hard_copy)    # hard_copy = [ 5, 3, 6 ]
print(id(lst) == id(hard_copy))
